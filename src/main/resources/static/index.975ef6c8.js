...
var _loginPage = require("./js/loginPage");
var _api = require("./js/API/api");

},{"./js/loginPage":"jopET","./js/API/api":"c7ya5"}],"jopET":[function(require,module,exports) {
...
parcelHelpers.export(exports, "renderSeats", ()=>renderSeats);
var _refsLoginPage = require("./refsLoginPage");
var _api = require("./API/api");
var _chairSvgrepoComSvg = require("../images/chair-svgrepo-com.svg");
var _chairSvgrepoComSvgDefault = parcelHelpers.interopDefault(_chairSvgrepoComSvg);
var _vectorSvg = require("../images/vector.svg");
...
async function dataForRender() {
    const dataPerChoise = await (0, _api.datas)();
    renderDatas(dataPerChoise);
}
async function renderDatas(datas) {
    const dataList = datas.map((data)=>{
        return `<li class="data-item"><span class='dataItem'>${data}</span></li>`;
    }).join("");
    (0, _refsLoginPage.refs).chosenData.innerHTML = dataList;
    (0, _refsLoginPage.refs).chosenData.addEventListener("click", moviesData);
}
async function moviesData(e) {
    (0, _refsLoginPage.refs).chairBox.classList.add("visually-hidden");
    const filmsPerData = await (0, _api.sessionBsByData)(e.target.textContent);
    chosenData = e.target.textContent;
    renderMovies(filmsPerData);
}
async function renderMovies(datas) {
    const dataList = datas.map((data)=>{
        if (chosenDataArr.roomId !== data.room.id) chosenDataArr.roomId = data.room.id;
        return `<li class="films-item">
        <span>${data.movie.name}</span><p class="time-item">${data.time.slice(0, 5)}</p>
      </li>`;
    }).join("");
    (0, _refsLoginPage.refs).filmsList.innerHTML = dataList;
    (0, _refsLoginPage.refs).filmBox.classList.remove("visually-hidden");
    (0, _refsLoginPage.refs).filmsList.addEventListener("click", sessionChairs);
}
async function sessionChairs(e) {
    if (e.target.nodeName !== "P") return;
    chosenTime = e.target.textContent.slice(0, 5);
    const seatsPerSession = await (0, _api.dataSeats)(chosenData, e.target.textContent);
    renderSeats(seatsPerSession);
}
async function renderSeats(datas) {
    chosenDataArr.seats = [];
    const dataList = datas.map((data)=>{
        let seatClass = "chair-item";
        let img = (0, _chairSvgrepoComSvgDefault.default);
        if (data.occupied) {
            seatClass = "chair-item-ocup";
            img = (0, _vectorSvgDefault.default);
        }
        return `<li class=${seatClass}>
      <img src=${img} alt="" class="chair-icon" width="50" height="50">
        <span class="number">${data.number}</span>
      </li>`;
    }).join("");
    (0, _refsLoginPage.refs).chairsList.innerHTML = dataList;
    (0, _refsLoginPage.refs).chairBox.classList.remove("visually-hidden");
    (0, _refsLoginPage.refs).chairsList.addEventListener("click", chosenSeats);
}
function chosenSeats(e) {
    if (e.target.nodeName !== "SPAN") return;
    e.target.parentNode.classList.toggle("active");
    const numberOfSeat = Number(e.target.textContent);
    if (chosenDataArr.seats.includes(numberOfSeat)) {
        const index = chosenDataArr.seats.indexOf(numberOfSeat);
        chosenDataArr.seats.splice(index, 1);
        renderResultList();
        if (chosenDataArr.seats.length === 0) {
            (0, _refsLoginPage.refs).resultListBox.textContent = "There are no chosen seats(((";
            (0, _refsLoginPage.refs).submitBtn.disabled = true;
        }
        if (chosenDataArr.seats.length !== 0) (0, _refsLoginPage.refs).submitBtn.disabled = false;
        return;
    }
    chosenDataArr.seats.push(numberOfSeat);
    if (chosenDataArr.seats.length !== 0) (0, _refsLoginPage.refs).submitBtn.disabled = false;
    renderResultList();
}
function renderResultList() {
    const resultList = chosenDataArr.seats.map((seat)=>{
        return ` <span class="chair">${seat}</span>`;
    }).join("");
    (0, _refsLoginPage.refs).resultListBox.innerHTML = resultList;
    (0, _refsLoginPage.refs).resultBox.classList.remove("visually-hidden");
}
(0, _refsLoginPage.refs).submitBtn.addEventListener("click", dataPost);
async function dataPost() {
    await (0, _api.dataPostBook)(chosenData, chosenTime, chosenDataArr);
    location.reload();
}

},{"./refsLoginPage":"g2Yb4","./API/api":"c7ya5","../images/chair-svgrepo-com.svg":"e7ZeX","../images/vector.svg":"5b0HQ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"g2Yb4":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "refs", ()=>refs);
const refs = {
    userBtn: document.querySelector(".auth-btn.login"),
    adminBtn: document.querySelector(".auth-btn.admin"),
    chosenData: document.querySelector(".data-list"),
    filmBox: document.querySelector(".films"),
    filmsList: document.querySelector(".films-list"),
    chairBox: document.querySelector(".chairs"),
    chairsList: document.querySelector(".chairs-list"),
    resultBox: document.querySelector(".result"),
    resultListBox: document.querySelector(".chairs-shosen"),
...
},{"./../utils.js":"5By4s","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"e7ZeX":[function(require,module,exports) {
module.exports = require("./helpers/bundle-url").getBundleURL("bLxZJ") + "chair-svgrepo-com.7390e14e.svg" + "?" + Date.now();
...
},{}],"5b0HQ":[function(require,module,exports) {
module.exports = require("./helpers/bundle-url").getBundleURL("bLxZJ") + "vector.bc6fa3f9.svg" + "?" + Date.now();

},{"./helpers/bundle-url":"lgJ39"}]},["1RB6v","8lqZg"], "8lqZg", "parcelRequire0977")
